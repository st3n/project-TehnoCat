...
class CommandParser():
    ...
    def __init__(self):
        self.add_pattern = re.compile(r'add (?:name:([^,]+))?(?:, *phone:([^,]+))?(?:, *email:([^,]+))?(?:, *address:([^,]+))?')
        self.change_pattern = re.compile('^change (.+?) (phone|email|address) \"?(.+?)\"? on \"?(.+?)\"?$')
    ...
    def parse_add_cmd(self, user_input):
        ...
        if add_match:
            ...
            return [ "add_contact", { "name" : name, "phone" : phone, "email" : email, "address" : address } ]
    ...
    def parse_change_cmd(self, user_input):
        ...
        if change_match:
            ...
            if property == "phone":
                return [ "change_phone", { "name" : name, "old_value" : old_value, "new_value" : new_value, } ]
            if property == "email":
                return [ "change_email", { "name" : name, "old_value" : old_value, "new_value" : new_value, } ]
            if property == "address":
                return [ "change_address", { "name" : name, "old_value" : old_value, "new_value" : new_value, } ]
    ...
    def parse_remove_cmd(self, user_input):
        ...
        if remove_match:
            ...
            if property == "phone":
                return [ "remove_phone", { "name" : name, "phone" : value, } ]
            if property == "email":
                return [ "remove_email", { "name" : name, "email" : value, } ]
            if property == "address":
                return [ "remove_address", { "name" : name, "address" : value, } ]
    ...
    def parse_add_address_cmd(self, user_input):
        ...
        if add_address_match:
            name = add_address_match.group(1).strip() if add_address_match.group(1) else ""
            if not name:
                raise ValueError #name is obligatory
            address = add_address_match.group(2).strip() if add_address_match.group(2) else ""
            if address:
    ...
    def parse_add_birthday_cmd(self, user_input):
        ...
        if add_birthday_match:
            name = add_birthday_match.group(1).strip() if add_birthday_match.group(1) else ""
            if not name:
                raise ValueError #name is obligatory
            birthday = add_birthday_match.group(2).strip() if add_birthday_match.group(2) else ""
            if birthday:
